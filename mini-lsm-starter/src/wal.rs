...
impl Wal {
    ...
    pub fn recover(path: impl AsRef<Path>, skiplist: &SkipMap<KeyBytes, Bytes>) -> Result<Self> {
        ...
        let mut i = 0;
        while i < len {
            let mut batch_size = u32::from_be_bytes(buffer[i..i + 4].try_into().unwrap());
            i += 4;
            let batch_start = i;
            while batch_size > 0 {
                ...
                i += 8;
                let value_len =
                    usize::from_be_bytes(buffer[i..i + USIZE_BYTES].try_into().unwrap());
                i += USIZE_BYTES;
            ...
            if checksum != crc32fast::hash(&buffer[batch_start..i]) {
    ...
    pub fn put_batch(&self, data: &[(KeySlice, &[u8])]) -> Result<()> {
        ...
        // Write the checksum for the body
        buf.extend(crc32fast::hash(&buf[4..]).to_be_bytes());
        buf_writer.write_all(&buf)?;
